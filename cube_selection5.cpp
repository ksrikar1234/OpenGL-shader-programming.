   ...
   for(auto triangle : this->triangles)
       {
            ...
            for(auto color :  triangle.vertex1_color) this->VERTEX_ARRAY_GL.push_back(generateColor(id)); 

            for(auto coord :  triangle.vertex2)       this->VERTEX_ARRAY_GL.push_back(coord); 
            for(auto color :  triangle.vertex2_color) this->VERTEX_ARRAY_GL.push_back(generateColor(id+1)); 

            for(auto coord :  triangle.vertex3)        this->VERTEX_ARRAY_GL.push_back(coord); 
            for(auto color :  triangle.vertex3_color)  this->VERTEX_ARRAY_GL.push_back(generateColor(id+2));                       
            ++id;
        }    
 ...
 */    
  ...
  //glBufferData(GL_ARRAY_BUFFER, sizeof(GeometryVertices), GeometryVertices, GL_STATIC_DRAW);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        cameraPos = glm::vec3(-right_key + left_key - X, - up_key + down_key - Y,   scroll_offset*20 + 20.0f); //  use (-scroll_offset + 2.0f) for zooming but distrort perspective so dont use it (Remember this)
